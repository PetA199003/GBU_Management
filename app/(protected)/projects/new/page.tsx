...
const shouldAutoSelect = (assessment: RiskAssessment, projectData: any, eventDate: Date, availableCriteria: any[]): boolean => {
  ...
  if (criteria.season && criteria.season.length > 0) {
    const detectedSeason = projectData.detectedSeason || getSeasonFromDate(eventDate);
    console.log('Season check:', detectedSeason, 'in', criteria.season);
    if (!criteria.season.includes(detectedSeason)) {
...
// Helper function to get German season name
const getGermanSeason = (season: 'spring' | 'summer' | 'autumn' | 'winter'): string => {
  ...
  const handleNextStep = () => {
    if (currentStep === 1) {
      ...
      if (globalRiskAssessments.length > 0) {
        ...
        // Prepare project data with detected season
        const projectDataWithSeason = {
          ...formData,
          customCriteria: customCriteriaValues,
          detectedSeason: formData.eventStart ? getSeasonFromDate(formData.eventStart) : null
        };
        
        console.log('Project data with season:', projectDataWithSeason);
        
        const autoSelected = globalRiskAssessments
          .filter(assessment => {
            const shouldSelect = shouldAutoSelect(assessment, projectDataWithSeason, formData.eventStart!, availableCriteria);
                  ...
                  {criteriaLoaded && availableCriteria.length > 0 && (
                    <div className="space-y-4">
                      <div className="border-t pt-4">
                        ...
                        {/* Automatische Jahreszeit-Erkennung */}
                        {formData.eventStart && (
                          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            <div className="flex items-center gap-2 mb-2">
                              <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                              <span className="text-sm font-medium text-blue-800">
                                Automatisch erkannte Jahreszeit
                              </span>
                            </div>
                            <p className="text-sm text-blue-700">
                              Basierend auf dem Veranstaltungsdatum ({format(formData.eventStart, 'dd.MM.yyyy', { locale: de })}) 
